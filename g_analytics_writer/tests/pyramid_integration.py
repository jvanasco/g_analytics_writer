...
class _TestSetup(_TestHarness):
    ...
    def test_pyramid_setup(self):
        ...
        if self._gwriter_use_comments is None:
            self.assertEqual(self.request.g_analytics_writer.use_comments, AnalyticsWriter.use_comments)
        else:
        ...
        if self._gwriter_single_push is None:
            self.assertEqual(self.request.g_analytics_writer.single_push, AnalyticsWriter.single_push)
        else:
        ...
        if self._gwriter_global_custom_data is None:
            self.assertEqual(self.request.g_analytics_writer.global_custom_data, AnalyticsWriter.global_custom_data)
        else:
            self.assertEqual(self.request.g_analytics_writer.global_custom_data, self._gwriter_global_custom_data)

        # possible overrides
        if self._gwriter_gtag_dimensions_strategy is None:
            self.assertEqual(self.request.g_analytics_writer.gtag_dimensions_strategy, AnalyticsWriter.gtag_dimensions_strategy)
        else:
